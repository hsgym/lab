...
# class CommonSubsetCreator(ABC):
"""
ファイルの読み書きや座標変換用の関数を持つ
"""
def read_crop_info_json(json_path:str, fov:int) -> Tuple[int, int, int, int]:
...
def write_crop_info_json(json_path:str, data_to_write) -> None:
...
def read_info_yaml(yaml_path:str) -> Tuple[int, int]:
    with open(yaml_path, 'r') as f:
        data = yaml.safe_load(f)
    return data['imgae_data']['width'],  data['imgae_data']['height']


def write_info_yaml(yaml_path, data_to_write) -> None:
...
def microns_to_pixel(x_micron:float, y_micron:float, microns_per_pixel:float, bbox_x:float, bbox_y:float) -> Tuple[float, float]:
    x_pixel = (x_micron - bbox_x) / microns_per_pixel
    y_pixel = (y_micron - bbox_y) / microns_per_pixel
    return x_pixel, y_pixel

def pixel_to_microns(x_pixel:float, y_pixel:float, microns_per_pixel:float, bbox_x:float, bbox_y:float) -> Tuple[float, float]:
